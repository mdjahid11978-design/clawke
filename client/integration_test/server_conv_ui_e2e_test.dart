...
class MockModelsApiService extends Mock implements ModelsApiService {}

class MockSkillsApiService extends Mock implements SkillsApiService {}

class FakeConvConfig extends Fake implements ConvConfig {}

class FakeSkillScope extends Fake implements SkillScope {}
...
void main() {
  ...
  setUpAll(() {
    ...
    registerFallbackValue(FakeSkillScope());
  });

  late AppDatabase db;
  late MockConfigApiService mockApi;
  late MockModelsApiService mockModelsApi;
  late MockSkillsApiService mockSkillsApi;
  ...
  setUp(() {
    ...
    mockModelsApi = MockModelsApiService();
    mockSkillsApi = MockSkillsApiService();
    ...
    when(
      () => mockModelsApi.listModels(any(), refresh: any(named: 'refresh')),
    ).thenAnswer(
      (_) async => const [
        GatewayModel(modelId: 'gpt-4', displayName: 'gpt-4', provider: 'mock'),
      ],
    );
    when(
      () => mockSkillsApi.listSkills(
        scope: any(named: 'scope'),
        locale: any(named: 'locale'),
      ),
    ).thenAnswer((_) async => const []);
    when(
      () => mockApi.saveConvConfig(any(), any()),
  ...
  group('Server-Authoritative Conversations UI E2E Tests', () {
    testWidgets(
      'UI Workflow: Create conversation passes UUID to Server then Local',
      (tester) async {
        // 1. 初始化依赖并注入
        await tester.pumpWidget(
          ProviderScope(
            overrides: [
              ...
              modelsApiServiceProvider.overrideWithValue(mockModelsApi),
              skillsApiServiceProvider.overrideWithValue(mockSkillsApi),
        ...
        // 点击“创建”按钮
        final saveBtnFinder = find.text('创建');
        await tester.tap(saveBtnFinder);
        await tester.pumpAndSettle(); // 等待弹窗退出列表刷新

        // 4. 验证会话已更新在侧边栏出现
        expect(find.text('Test Delay Create Event'), findsWidgets);

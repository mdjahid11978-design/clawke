...
void main() {
  ...
  group('Server-Authoritative Conversations Sync E2E Test', () {
    testWidgets('syncFromServer: Full sync flow (Create, Update, Delete)', (
      tester,
    ) async {
      ...
      ).thenAnswer((_) async => serverConvs);
      ...
      expect(
        localList.length,
        2,
        reason: 'Should have deleted one, kept one, added one',
      );

      final updateConv = localList.firstWhere(
        (c) => c.conversationId == 'conv_update',
      );
      expect(updateConv.name, 'New Name');
      expect(updateConv.isPinned, 1);
      expect(updateConv.isMuted, 1);

      final newConv = localList.firstWhere(
        (c) => c.conversationId == 'conv_new',
      );
      expect(newConv.name, 'Newly Created');
      expect(newConv.type, 'group');

      final deletedConv = localList
          .where((c) => c.conversationId == 'conv_delete')
          .toList();
      expect(deletedConv.isEmpty, true);
    });

    testWidgets(
      'createConversation passes client-generated UUID to server first, then local (delayed creation)',
      (tester) async {
        when(
    ...
    testWidgets('deleteConversation deletes on server, then local', (
      tester,
    ) async {
      await dao.upsertConversation(
        const ConversationsCompanion(
      ...
      when(
        () => mockApi.deleteConversation('delete_me'),
    ...
    testWidgets('togglePin syncs with server, then updates local', (
      tester,
    ) async {
      await dao.upsertConversation(
        const ConversationsCompanion(
      ...
      when(
        () => mockApi.updateConversation('pin_me', isPinned: true),
      ).thenAnswer((_) async => true);

...
void main() {
  ...
  testWidgets('task management shows cached definitions before remote sync', (
    tester,
  ) async {
    ...
    final seedTaskApi = _FakeTasksApiService()
    ...
    expect(find.text('远端天气任务'), findsNothing);

    remoteCompleter.complete([
      _task(
        'remote-weather',
        '远端天气任务',
        lastRun: const TaskRun(
          id: 'run-1',
          taskId: 'remote-weather',
          startedAt: '2026-04-25T08:00:00Z',
          finishedAt: '2026-04-25T08:01:00Z',
          status: 'success',
          outputPreview: 'rain',
        ),
      ),
    ...
    final cached = await taskRepo.getTasks('hermes');
    expect(cached.single.id, 'remote-weather');
    expect(cached.single.lastRun, isNull);

    await _disposeSubject(tester);
  });

  testWidgets('skill management shows cached translation and edits source', (
    tester,
  ) async {
    final db = AppDatabase.forTesting(NativeDatabase.memory());
    ...
    final seedSkillsApi = _FakeSkillsApiService()
      ..listResponse = [
        _skill(
...
ManagedTask _task(String id, String name, {TaskRun? lastRun}) {
  return ManagedTask(
    id: id,
    accountId: 'hermes',
    agent: 'Hermes',
    ...
    prompt: '查询杭州天气',
    enabled: true,
    status: lastRun?.status == 'running' ? 'running' : 'active',
